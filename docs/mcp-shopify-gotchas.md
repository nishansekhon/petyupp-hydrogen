...
**Concrete: Himalayan Cheese.** Plain `himalayan-gourmet-cheese-chew` (originally "Pack of 4 - Small", $24.99) and the flavored 3.5oz packs ($18.99) are different products at different price tiers, but they shared "3.5oz" wording. Phase 5's first-pass migration collapsed the flavored variants into Size="Small 3.5oz" alongside the legacy Plain — sharing one Size value. Symptom on the PDP: every Flavor chip rendered enabled at exactly one Size and disabled at every other, because that was literally the only Size where a flavored variant existed. Visually surprising even though it was accurate to the data.

**Resolution (2026-04-25):** the 8 flavored variants were re-tiered from Size="Small 3.5oz" to Size="Medium" via `productVariantsBulkUpdate`, joining the existing Plain Medium under the same option-value GID (no new Size value created). Plain "Small 3.5oz" remains as the lone variant at that Size value (it's a different physical pack — Pack of 4 - Small — that just happens to share the "3.5oz" substring). PdpBuyBox's option-group hide logic (kicks in when a Size has only one valid Flavor) keeps the UX clean either way.

**Rule for future family consolidations:** before merging two source-handle tokens into one Size value, audit whether they represent the same segment at the same price tier. If they don't, model them as **separate Size values** (e.g. one Size value per price tier) even if the substring is identical, OR use a separate option axis to disambiguate (e.g. add a "Tier" or "Pack" option). Catching this at migration time avoids a later mutation pass like the Apr 25 re-tier.
